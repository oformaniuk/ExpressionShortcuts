...
namespace Microsoft.CSharp.Expressions
{
    ...
    public sealed class AsyncCSharpExpression<TDelegate> : AsyncLambdaCSharpExpression
    {
        ...
        private Expression RewriteBody(AsyncMethodBuilderInfo builderInfo, ParameterExpression stateVar, ParameterExpression builderVar, ParameterExpression stateMachineVar, out IEnumerable<ParameterExpression> variables)
        {
            ...
            if (Body.Type != typeof(void) && builderVar.Type.GetTypeInfo().IsGenericType /* if not ATMB<T>, no result assignment needed */)
            {
    ...
    partial class CSharpExpression
    {
        ...
        public static AsyncLambdaCSharpExpression AsyncLambda(Expression body, IEnumerable<ParameterExpression> parameters)
        {
            ...
            return CreateAsyncLambda(GetDelegateType(types), body, parameterList);
        }
        ...
        private static void ValidateAsyncLambdaArgs(Type delegateType, ref Expression body, ReadOnlyCollection<ParameterExpression> parameters)
        {
            ...
            {
                if (resultType != typeof(void) && !TypeUtils.AreReferenceAssignable(resultType, body.Type) && !ExpressionUtils.TryQuote(resultType, ref body))
                {
        ...
        {
            ExpressionUtils.RequiresCanRead(parameter, "parameters");
