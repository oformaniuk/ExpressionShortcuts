...
namespace Microsoft.CSharp.Expressions
{
    ...
    {
        public static Exception UnhandledBinary(CSharpExpressionType binaryType)
        {
            throw new Exception($"{nameof(UnhandledBinary)}: {binaryType}");
        }

        public static Exception IncorrectNumberOfMethodCallArguments(LambdaExpression conversion)
        {
            throw new Exception($"{nameof(IncorrectNumberOfMethodCallArguments)}: {conversion.Name}");
        }
        
        public static Exception IncorrectNumberOfMethodCallArguments(MethodInfo conversion)
        {
            throw new Exception($"{nameof(IncorrectNumberOfMethodCallArguments)}: {conversion.Name}");
        }

        public static Exception OperandTypesDoNotMatchParameters(CSharpExpressionType nodeType, string toString)
        {
            throw new Exception($"{nameof(OperandTypesDoNotMatchParameters)}: {nodeType} - {toString}");
        }

        public static Exception UnhandledUnary(CSharpExpressionType unaryType)
        {
            throw new Exception($"{nameof(UnhandledUnary)}: {unaryType}");
        }
        ...
        public static Exception DuplicateVariable(ParameterExpression parameter)
        {
            throw new Exception($"{nameof(DuplicateVariable)}: {parameter.Name}");
        }

        public static Exception LambdaTypeMustBeDerivedFromSystemDelegate()
        {
            throw new Exception(nameof(LambdaTypeMustBeDerivedFromSystemDelegate));
        }

        public static Exception IncorrectNumberOfLambdaDeclarationParameters()
        {
            throw new Exception(nameof(IncorrectNumberOfLambdaDeclarationParameters));
        }

        public static Exception ParameterExpressionNotValidAsDelegate(Type parameterType, Type type)
        {
            throw new Exception($"{nameof(ParameterExpressionNotValidAsDelegate)}: {parameterType} - {type}");
        }

        public static Exception ExpressionTypeDoesNotMatchReturn(Type bodyType, Type methodReturnType)
        {
            throw new Exception($"{nameof(ExpressionTypeDoesNotMatchReturn)}: {bodyType} - {methodReturnType}");
        }

        public static Exception ArgumentOutOfRange(string indexName)
        {
            throw new IndexOutOfRangeException(indexName);
        }

        public static Exception ExpressionTypeDoesNotMatchParameter(Type operandType, Type firstParamParameterType)
        {
            throw new Exception(nameof(ExpressionTypeDoesNotMatchParameter));
        }

        public static Exception ExpressionTypeDoesNotMatchAssignment(Type resourceType, Type variableType)
        {
            throw new Exception($"{nameof(ExpressionTypeDoesNotMatchAssignment)} - {resourceType} - {variableType}");
        }

        public static Exception ArgumentMustBeArray()
        {
            throw new Exception(nameof(ArgumentMustBeArray));
        }

        public static Exception IncorrectNumberOfIndexes()
        {
            throw new Exception(nameof(IncorrectNumberOfIndexes));
        }

        public static Exception ArgumentMustBeArrayIndexType()
        {
            throw new Exception(nameof(ArgumentMustBeArrayIndexType));
        }

        public static Exception LabelTypeMustBeVoid()
        {
            throw new Exception(nameof(LabelTypeMustBeVoid));
        }

        public static Exception MemberNotFieldOrProperty(MemberInfo member)
        {
            throw new Exception($"{nameof(MemberNotFieldOrProperty)} - {member.Name}");
        }

        public static Exception FieldInfoNotDefinedForType(Type fieldDeclaringType, string fieldName, Type type)
        {
            throw new Exception(nameof(FieldInfoNotDefinedForType));
        }

        public static Exception InstanceFieldNotDefinedForType(string fieldName, Type type)
        {
            throw new Exception($"{nameof(InstanceFieldNotDefinedForType)} - {type.Name} - {fieldName}");
        }

        public static Exception PropertyNotDefinedForType(PropertyInfo property, Type type)
        {
            throw new Exception($"{nameof(PropertyNotDefinedForType)} - {type.Name} - {property.Name}");
        }

        public static Exception InstancePropertyNotDefinedForType(string propertyName, Type type)
        {
            throw new Exception($"{nameof(InstancePropertyNotDefinedForType)} - {type.Name} - {propertyName}");
        }

        public static Exception ArgumentMustBeBoolean()
        {
            throw new ArgumentException(nameof(ArgumentMustBeBoolean));
        }

        public static Exception PropertyDoesNotHaveAccessor(PropertyInfo lengthOrCount)
        {
            throw new Exception($"{nameof(PropertyDoesNotHaveAccessor)} - {lengthOrCount.Name}");
        }
        
        public static Exception PropertyDoesNotHaveAccessor(MemberInfo lengthOrCount)
        {
            throw new Exception($"{nameof(PropertyDoesNotHaveAccessor)} - {lengthOrCount.Name}");
        }

        public static Exception PropertyNotDefinedForType(MethodInfo indexAccessor, Type objectType)
        {
            throw new Exception($"{nameof(PropertyNotDefinedForType)} - {indexAccessor.Name} - {objectType.Name}");
        }
    }
            ...
            if (binaryType != CSharpExpressionType.Assign && binaryType != CSharpExpressionType.NullCoalescingAssign)
            {
                ...
                else if (IsCSharpSpecificCompoundNumeric(leftType))
                {
                    ...
                    if (method == null && leftConversion == null && finalConversion == null)
                    {
                        ...
                        var nonNullLeftType = leftType.GetNonNullableType();//.GetTypeInfo();
                        var intermediateType = nonNullLeftType.GetTypeInfo().IsEnum ? nonNullLeftType.GetEnumUnderlyingType() : typeof(int);
