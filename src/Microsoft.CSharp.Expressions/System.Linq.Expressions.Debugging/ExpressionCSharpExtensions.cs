...
namespace System.Linq.Expressions
{
    ...
    public static class ExpressionCSharpExtensions
    {
        ...
        {
            return ToCSharp(type, new HashSet<string>(namespaces ?? ArrayEx.Empty<string>()));
        }
        ...
        class CSharpPrinter : ExpressionVisitor, ICSharpPrintingVisitor
        {
            ...
            protected override Expression VisitNew(NewExpression node)
            {
                ...
                else
                {
                    ...
                    if (node.Members != null || (node.Type.GetTypeInfo().IsDefined(typeof(CompilerGeneratedAttribute)) && node.Type.Name.StartsWith("<>f__AnonymousType")))
                    {
                        Out("{");
    ...
    internal static class CSharpLanguageHelpers
    {
        ...
        {
            if (method.GetGenericArguments().Any(t => t.GetTypeInfo().IsDefined(typeof(CompilerGeneratedAttribute)) && t.Name.StartsWith("<>f__AnonymousType")))
            {
                return true;
            }
