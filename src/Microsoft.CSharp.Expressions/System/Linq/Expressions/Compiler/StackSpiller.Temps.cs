...
#if NET452 || NET451
            internal void AddArguments(Expression expression)
            {
                var arguments = (IList<Expression>) expression.GetType()
                    .GetField("_arguments", BindingFlags.Instance | BindingFlags.NonPublic)!
                    .GetValue(expression); 
